"""
FIR PII Extractor — ChatGPT-style Final Version
Handles Hindi + English FIRs, from PDF or pasted text.
"""
...
# Known dictionaries
INDIAN_STATES = [
    "Maharashtra","Uttar Pradesh","Delhi","Karnataka","Bihar","Gujarat",
    "Tamil Nadu","Madhya Pradesh","Rajasthan","West Bengal","Punjab","Haryana"
]
DISTRICTS_UP = ["मेरठ","लखनऊ","वाराणसी","कानपुर","नोएडा","गाजियाबाद"]
ACTS = [
    "IPC 1860","CrPC 1973","Arms Act 1959","NDPS Act 1985",
    "IT Act 2000","Maharashtra Police Act 1951"
]

PLACEHOLDER_BAD = {"नाव","Name","Type","Address","of P.S.","then Name of P.S."}
...
def filter_placeholders(val):
    if not val: return None
    v = val.strip()
    return None if v in PLACEHOLDER_BAD or len(v) < 2 else v
...
def extract_pii(text: str) -> dict:
    ...
    # District
    m = re.search(r"(?:District|जिला|जिल्हा)\s*[:\-]?\s*([A-Za-z\u0900-\u097F ]{2,40})", text)
    if m: out["dist_name"] = fuzzy_match(m.group(1), DISTRICTS_UP)

    # Police Station
    m = re.search(r"(?:Police Station|P\.S\.|थाना|पोलीस ठाणे)\s*[:\-]?\s*([A-Za-z\u0900-\u097F ]{2,50})", text)
    if m: out["police_station"] = filter_placeholders(fix_broken_devanagari(m.group(1)))
    ...
    valid_secs = [s for s in flat if int(s) >= 20]  # drop 1,2,3,4 noise
    out["under_sections"] = list(sorted(set(valid_secs))) if valid_secs else None
    ...
    if out["under_sections"]:
        ...
        elif out["under_acts"] and "IT Act 2000" in out["under_acts"]:
            out["revised_case_category"] = "CYBER_CRIME"
        else:
    ...
    if NER_PIPE:
        ...
        for e in ents:
            if e["entity_group"] == "PER" and not out["name"]:
                out["name"] = filter_placeholders(e["word"])
                out["oparty"] = "Complainant"
            elif e["entity_group"] in ["LOC","ORG"] and not out["address"]:
                out["address"] = filter_placeholders(fix_broken_devanagari(e["word"]))

    # Cleanup placeholders
    for k in ["name","address","dist_name","police_station"]:
        out[k] = filter_placeholders(out[k])
...
# ---------- Streamlit UI ----------
st.title("FIR PII Extractor — Final Version")
st.write("Upload FIR PDFs or paste text. Extracts clean, ChatGPT-style PII JSON.")
